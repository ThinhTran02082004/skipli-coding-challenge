...
// khởi tạo transporter cho nodemailer nếu có cấu hình gmail hợp lệ
...
// hàm gửi mã otp qua email bằng nodemailer
export async function sendEmailOtp(toEmail: string, code: string): Promise<boolean> {
  ...
  // thực hiện gửi thư nếu transporter khả dụng
...
// hàm gửi email thông báo tài khoản mới cho học sinh
export async function sendStudentCredentialEmail(toEmail: string, name: string, phone: string): Promise<boolean> {
  ...
  if (transporter && emailUser) {
    ...
    // chế độ mô phỏng log thông báo chào mừng
